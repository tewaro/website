...
main = hakyll $ do
    ...
    tagsRules tags $ \tagStr tagsPattern -> do
        route idRoute
        compile $ do
            postsTags <- loadAll "posts/*" >>= recentFirst
            draftsTags <- loadAll "drafts/*" >>= recentFirst
            let postsCtx = constField "title" tagStr
                    <> listField "posts" postCtx (return postsTags)
                    <> listField "drafts" postCtx (return draftsTags)
                    <> defaultContext
            makeItem ""
